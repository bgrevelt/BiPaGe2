...
class NestedDataTypeBigEndian(unittest.TestCase, IntegrationTest):
...
#include "nested_data_type_big_endian_integrationtest_generated.h"
#include <iostream>
#include <vector>
#include <limits>
#include "../check.hpp"
...
{            
    ...
    p = serialize(p, naive_swap(static_cast<std::int32_t>(8)));    // Bar::f1
    p = serialize(p, naive_swap(static_cast<std::int32_t>(-20)));   // Foo:field1
    p += sizeof(float);                                             // Foo:padding
    p = serialize(p, naive_swap(static_cast<std::uint16_t>(30)));   // Foo::field3
    p = serialize(p, foo_field4_values);                            // Foo::field4
    p = serialize(p, naive_swap(1.234));                            // Foo::field5
    p = serialize(p, naive_swap(2.345));                            // Bar:f3
...
void test_bar_builder()
{
    ...
    p = serialize(p, naive_swap(Bar_field1));
    p = serialize(p, naive_swap(Foo_field1));
    p += sizeof(float);
    p = serialize(p, naive_swap(Foo_field3));
    p = serialize(p, Foo_field4);
    p = serialize(p, naive_swap(Foo_field5));
    p = serialize(p, naive_swap(Bar_field3));
